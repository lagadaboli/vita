...
final class DashboardViewModel {
    ...
    var watchStatus: ConnectionStatus = .notConfigured
    ...
    func load(from appState: AppState) {
        ...
        let watchWindowStart = calendar.date(byAdding: .day, value: -2, to: now) ?? now
        ...
        if let samples = try? appState.healthGraph.querySamples(type: .stepCount, from: dayStart, to: now) {
            steps = Int(samples.reduce(0) { $0 + $1.value })
        }

        // Compute Apple Watch status from recent live metrics.
        if appState.dataMode != .live {
            watchStatus = .notConfigured
        } else {
            var latestTimestamps: [Date] = []
            if let samples = try? appState.healthGraph.querySamples(type: .hrvSDNN, from: watchWindowStart, to: now),
               let last = samples.last {
                latestTimestamps.append(last.timestamp)
            }
            if let samples = try? appState.healthGraph.querySamples(type: .restingHeartRate, from: watchWindowStart, to: now),
               let last = samples.last {
                latestTimestamps.append(last.timestamp)
            }
            if let samples = try? appState.healthGraph.querySamples(type: .sleepAnalysis, from: watchWindowStart, to: now),
               let last = samples.last {
                latestTimestamps.append(last.timestamp)
            }
            if let samples = try? appState.healthGraph.querySamples(type: .stepCount, from: watchWindowStart, to: now),
               let last = samples.last {
                latestTimestamps.append(last.timestamp)
            }

            if let latest = latestTimestamps.max() {
                watchStatus = now.timeIntervalSince(latest) <= 6 * 3600 ? .connected : .disconnected
            } else {
