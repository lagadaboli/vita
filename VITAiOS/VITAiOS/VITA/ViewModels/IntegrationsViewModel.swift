...
final class IntegrationsViewModel {
    ...
    func load(from appState: AppState) {
        ...
        // Apple Watch / HealthKit status from live data.
        if appState.dataMode != .live {
            ...
            if let hrv = try? appState.healthGraph.querySamples(type: .hrvSDNN, from: watchWindowStart, to: now),
               let last = hrv.last {
                watchHRV = last.value
            ...
            if let hr = try? appState.healthGraph.querySamples(type: .restingHeartRate, from: watchWindowStart, to: now),
               let last = hr.last {
                watchHR = last.value
                latestTimestamps.append(last.timestamp)
            } else {
            ...
            if let sleep = try? appState.healthGraph.querySamples(type: .sleepAnalysis, from: watchWindowStart, to: now),
               let last = sleep.last {
                latestTimestamps.append(last.timestamp)
            }

            if let stepSamples = try? appState.healthGraph.querySamples(type: .stepCount, from: dayStart, to: now) {
                watchSteps = Int(stepSamples.reduce(0) { $0 + $1.value })
